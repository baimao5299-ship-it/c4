...
```powershell
.\scripts\deploy.ps1 -Server 23.251.32.239 -User root -RemoteDir /www/c4 -Domain qingyutian.duckdns.org -AppName 晴雨天
```

确认服务器地址、SSH 用户、端口和本机 SSH 登录无误后执行：

```powershell
.\scripts\deploy.ps1 -Server 23.251.32.239 -User root -RemoteDir /www/c4 -Domain qingyutian.duckdns.org -AppName 晴雨天 -Apply
```
...
5. 首次部署把 PostgreSQL 数据固定在 `<RemoteDir>/deploy/data/pg`（当前服务器为 `/www/c4/deploy/data/pg`），并固定 Compose 项目名 `c4`；后续 release 切换不会换库或创建第二套项目。
...
- 配置备份：定期备份服务器 `current/.env`（离线保管）和 `<RemoteDir>/deploy/data/pg`（当前为 `/www/c4/deploy/data/pg`），不要把它们提交到仓库。
