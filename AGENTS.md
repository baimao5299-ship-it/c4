...
**Generated:** 2026-08-22 · **Commit:** dc88cd0 · **Branch:** main
...
internal/
  ...
  billing/ usage/ pricing/ notify/ invalidate/ credential/ sdkbridge/ protoconv/ domain/ config/ worker/ snapshot/ auth/ server/
...
| 计费/定价 | `internal/billing`、`internal/pricing`（换算系 ×1e5 与 ×1e11 禁混用） |
...
- **测试**：testify **只 require 不 assert**；PG 测试真实 PostgreSQL（pgxmock 仅两个遗留文件，勿扩展）；全仓 `t.Parallel()`=0 串行；固定 `time.Date(2026,8,...)` 注入时间；channel 屏障替代 sleep（参考 codex_test.go runWave）
...
- **worker 注册序=反序排空语义**：billFlusher 在 listener/authSync 之前注册（main.go:477-486），动顺序前读注释
