...
type Rule struct {
	...
	// Then holds the value of the "then" field.
...
func (*Rule) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case rule.FieldWhen, rule.FieldThen:
			values[i] = new([]byte)
		...
		case rule.FieldUpdatedAt, rule.FieldDeletedAt, rule.FieldCreatedAt:
...
func (_m *Rule) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case rule.FieldThen:
			if value, ok := values[i].(*[]byte); !ok {
				return fmt.Errorf("unexpected type %T for field then", values[i])
			} else if value != nil && len(*value) > 0 {
				if err := json.Unmarshal(*value, &_m.Then); err != nil {
					return fmt.Errorf("unmarshal field then: %w", err)
				}
			}
...
func (_m *Rule) String() string {
	...
	builder.WriteString(fmt.Sprintf("%v", _m.Then))
	builder.WriteString(", ")
