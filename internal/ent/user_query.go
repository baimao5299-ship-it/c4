...
func (_q *UserQuery) QueryKeys() *KeyQuery {
...
// QueryTempBalances chains the current query on the "temp_balances" edge.
func (_q *UserQuery) QueryTempBalances() *TempBalanceQuery {
	query := (&TempBalanceClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryGroupAssignments chains the current query on the "group_assignments" edge.
func (_q *UserQuery) QueryGroupAssignments() *GroupAssignmentQuery {
...
func (_q *UserQuery) WithKeys(opts ...func(*KeyQuery)) *UserQuery {
	query := (&KeyClient{config: _q.config}).Query()
...
// WithTempBalances tells the query-builder to eager-load the nodes that are connected to
// the "temp_balances" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithTempBalances(opts ...func(*TempBalanceQuery)) *UserQuery {
	query := (&TempBalanceClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withTempBalances = query
	return _q
}

// WithGroupAssignments tells the query-builder to eager-load the nodes that are connected to
// the "group_assignments" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithGroupAssignments(opts ...func(*GroupAssignmentQuery)) *UserQuery {
	query := (&GroupAssignmentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withGroupAssignments = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withTempBalances; query != nil {
		if err := _q.loadTempBalances(ctx, query, nodes,
			func(n *User) { n.Edges.TempBalances = []*TempBalance{} },
			func(n *User, e *TempBalance) { n.Edges.TempBalances = append(n.Edges.TempBalances, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withGroupAssignments; query != nil {
		if err := _q.loadGroupAssignments(ctx, query, nodes,
			func(n *User) { n.Edges.GroupAssignments = []*GroupAssignment{} },
			func(n *User, e *GroupAssignment) { n.Edges.GroupAssignments = append(n.Edges.GroupAssignments, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadKeys(ctx context.Context, query *KeyQuery, nodes []*User, init func(*User), assign func(*User, *Key)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(key.FieldUserID)
	}
...
func (_q *UserQuery) loadTempBalances(ctx context.Context, query *TempBalanceQuery, nodes []*User, init func(*User), assign func(*User, *TempBalance)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(tempbalance.FieldUserID)
	}
	query.Where(predicate.TempBalance(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.TempBalancesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *UserQuery) loadGroupAssignments(ctx context.Context, query *GroupAssignmentQuery, nodes []*User, init func(*User), assign func(*User, *GroupAssignment)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(groupassignment.FieldUserID)
	}
	query.Where(predicate.GroupAssignment(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.GroupAssignmentsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
