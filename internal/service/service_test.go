...
func TestCreateGroupFlow(t *testing.T) {
	...
	g, err := svc.CreateGroup(context.Background(), "g1", domain.GroupVisibilityPublic, nil)
	require.NoError(t, err)
	require.Equal(t, domain.GroupVisibilityPublic, g.Visibility, "visibility 落库")
	g2, err := svc.CreateGroup(context.Background(), "g2", domain.GroupVisibilityPrivate, nil)
	require.NoError(t, err)
...
func TestBatchUpdateGroups(t *testing.T) {
	...
	svc := &Service{store: fs, inv: rec, log: nil}
	ctx := context.Background()
	g, err := svc.CreateGroup(ctx, "g1", domain.GroupVisibilityPublic, nil)
	require.NoError(t, err)
	name := "renamed"
	before := rec.total()
