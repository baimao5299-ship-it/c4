...
func TestCreateGroupNameConflict(t *testing.T) {
	...
	_, err := svc.CreateGroup(ctx, "dup-g", domain.GroupVisibilityPublic)
	require.NoError(t, err)

	_, err = svc.CreateGroup(ctx, "dup-g", domain.GroupVisibilityPrivate)
	require.ErrorIs(t, err, ErrConflict, "重复 name → ErrConflict（409）")
	require.Contains(t, err.Error(), `name="dup-g"`, "409 消息含冲突详情")

	g, err := svc.CreateGroup(ctx, "other-g", domain.GroupVisibilityPublic)
	require.NoError(t, err)
	g.Name = "dup-g"
