...
func TestPostRulesBatchDelete(t *testing.T) {
	...
	for i := 1; i <= 3; i++ {
		rec := do(http.MethodPost, "/admin/rules", `{
			...
			"when":{"kind":"error"},"then":{"status":"unhealthy"}}`)
	...
	rec := do(http.MethodPost, "/admin/rules/batch-delete", `{"ids":[`+itoa(ids[0])+`,`+itoa(ids[1])+`]}`)
	...
	// 重复 ids 去重 → {"deleted":1}
	rec = do(http.MethodPost, "/admin/rules/batch-delete", `{"ids":[`+itoa(ids[2])+`,`+itoa(ids[2])+`]}`)
	...
	// 空 ids → 400
	rec = do(http.MethodPost, "/admin/rules/batch-delete", `{"ids":[]}`)
	...
	rec = do(http.MethodPost, "/admin/rules/batch-delete", string(body))
...
func TestPostRulesBatchDeleteMissing(t *testing.T) {
	...
	rec := do(http.MethodPost, "/admin/rules/batch-delete", `{"ids":[999]}`)
