...
func TestPostTemplatesBatchDelete(t *testing.T) {
	...
	for _, name := range []string{"t1", "t2", "t3"} {
		rec := do(http.MethodPost, "/admin/templates", `{"name":"`+name+`","base_url":"https://api.openai.com/v1","default_format":"openai-chat"}`)
...
// 非法 default_format 400 / 空 fields 400。
func TestPostTemplatesBatchUpdate(t *testing.T) {
	...
	// 非法 default_format → 400（service validateTemplatePatch）
	rec = do(http.MethodPost, "/admin/templates/batch-update", `{"ids":[1],"fields":{"default_format":"bogus"}}`)
	require.Equal(t, 400, rec.Code, "invalid default_format: %s", rec.Body.String())
