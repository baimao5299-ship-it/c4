...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'price_variants_effect_at_least_one') THEN
    ALTER TABLE price_variants ADD CONSTRAINT price_variants_effect_at_least_one
