...
// 会话锁内顺序执行（并行即成环）。事务纪律：BEGIN → SET LOCAL sync_commit=off
// → 执行 → marked==batch 计数比对（不齐 = 并发标记，整事务回滚）→ COMMIT，
// 外层毒行梯子重试 ≤settleMaxAttempts 次、耗尽隔离队头越行。usage_logs 明细
// 唯一写者仍是 usage flusher（InsertBatch）；本文件只做消费。游标取批/纯标记/
...
// 单语句单事务原子完成 取批→条件扣→透支补刀→标记。limit<=0 → 零结果 no-op。
// 毒行梯子随迁（oracle 必改 #2）：失败重试 ≤settleMaxAttempts 次，耗尽后隔离
// 该车道队头行（MarkBilledBulk 写销 + Quarantined 计数返回）——确定性毒行纯
// LIMIT 重试永不收敛，游标越过继承「游标永不卡死」不变量。整个调用受
// settleTimeout per-query 超时约束（ctx 截止 → 回滚重放，不隔离）。
func (r *BillingRepo) SettleBalanceBatch(ctx context.Context, limit int) (domain.SettlementSummary, error) {
	return r.settleBatch(ctx, limit, settleBalancePlan)
}

// SettleFefoBatch Temp 车道结算一个窗口（≤limit 行，temp-active 用户）：集合化
// FEFO 消耗 + 差额透支补刀 + 标记一体（D7）。事务纪律与毒行梯子同
// SettleBalanceBatch。
func (r *BillingRepo) SettleFefoBatch(ctx context.Context, limit int) (domain.SettlementSummary, error) {
	return r.settleBatch(ctx, limit, settleFefoPlan)
}
...
// 约束 23xxx）→ 重试 ≤K 次耗尽后隔离该车道队头行——ProbeLaneHead 只读定位 →
...
func (r *BillingRepo) settleBatch(ctx context.Context, limit int, plan settlePlan) (domain.SettlementSummary, error) {
	if limit <= 0 {
	...
	for attempt := 0; attempt < settleMaxAttempts; attempt++ {
		res, err := r.settleOnce(ctx, limit, plan.sqlText)
		if err == nil {
			return res, nil
		}
		if ctx.Err() != nil {
	...
	head, ok, err := r.ProbeLaneHead(ctx, plan.probeSQL)
	if err != nil {
...
// ProbeLaneHead 毒行梯子只读探针（probeSQL = 对应车道 batch 谓词同构查询）：
// 返回该车道批队头行 id；空批 → ok=false。非事务读（无锁无副作用），仅供隔离
// 决策。
func (r *BillingRepo) ProbeLaneHead(ctx context.Context, probeSQL string) (id int64, ok bool, err error) {
	rows, err := r.queryRows(ctx, probeSQL, nil)
	if err != nil {
		return 0, false, err
	}
	defer rows.Close()
...
func (r *BillingRepo) settleOnce(ctx context.Context, limit int, sqlText string) (domain.SettlementSummary, error) {
	if r.pool != nil {
		return settlePGX(ctx, r.pool, limit, sqlText)
	}
	return settleEnt(ctx, r.driver, limit, sqlText)
}
...
func settlePGX(ctx context.Context, pool *pgxpool.Pool, limit int, sqlText string) (domain.SettlementSummary, error) {
	...
	res, err := runSettleStmt(ctx, &pgxSettleTx{tx: tx}, sqlText, limit)
	if err != nil {
...
// 构建器同事务连接（WithTx 同构）。
func settleEnt(ctx context.Context, drv dialect.Driver, limit int, sqlText string) (domain.SettlementSummary, error) {
	tx, err := drv.Tx(ctx)
	if err != nil {
		return domain.SettlementSummary{}, err
	}
	defer tx.Rollback() // nolint:errcheck // Commit 成功后 Rollback 返回 ErrTxDone，忽略
	res, err := runSettleStmt(ctx, &entSettleTx{drv: &txDriver{tx: tx, drv: drv}}, sqlText, limit)
	if err != nil {
		return domain.SettlementSummary{}, err
	}
...
// runSettleStmt 结算语句编排（两载体单一实现）：sync_commit 让渡 → 执行 → 扫描
// → marked==batch 计数比对守卫（不齐 = 他方消费者已抢标同批行 → errConcurrentMark
// 使整事务回滚——markBilledExec Σ守卫的语句化迁移）。
func runSettleStmt(ctx context.Context, exe settleTx, sqlText string, limit int) (domain.SettlementSummary, error) {
	...
	rows, err := exe.QueryRows(ctx, sqlText, []any{limit})
	if err != nil {
		return domain.SettlementSummary{}, err
	}
