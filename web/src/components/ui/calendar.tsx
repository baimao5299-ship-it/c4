  ...
  return (
    <DayPicker
      ...
      classNames={{
        ...
        // 中间段背景：muted（28% 深灰）叠玻璃面板上几乎不可见——浅色用浅青
        // （cyan-500 10%），深色沿用表格选中行蓝色系（table.tsx 已验证参数）
        range_middle: cn(
          "rounded-none bg-[color:color-mix(in_srgb,#06b6d4_10%,transparent)] dark:bg-[color:color-mix(in_srgb,#2997ff_14%,transparent)]",
...
function CalendarDayButton({
  ...
  return (
    <Button
      ...
      className={cn(
        "cn-calendar-day-button relative isolate z-10 flex aspect-square size-auto w-full min-w-(--cell-size) flex-col gap-1 border-0 leading-none font-normal group-data-[focused=true]/day:relative group-data-[focused=true]/day:z-10 group-data-[focused=true]/day:border-ring group-data-[focused=true]/day:ring-[3px] group-data-[focused=true]/day:ring-ring/50 data-[range-end=true]:rounded-(--cell-radius) data-[range-end=true]:rounded-r-(--cell-radius) data-[range-end=true]:bg-primary data-[range-end=true]:text-primary-foreground data-[range-middle=true]:rounded-none data-[range-middle=true]:bg-[color:color-mix(in_srgb,#06b6d4_10%,transparent)] data-[range-middle=true]:text-foreground data-[range-start=true]:rounded-(--cell-radius) data-[range-start=true]:rounded-l-(--cell-radius) data-[range-start=true]:bg-primary data-[range-start=true]:text-primary-foreground data-[selected-single=true]:bg-primary data-[selected-single=true]:text-primary-foreground dark:data-[range-middle=true]:bg-[color:color-mix(in_srgb,#2997ff_14%,transparent)] dark:hover:text-foreground [&>span]:text-xs [&>span]:opacity-70",
        defaultClassNames.day,
        className
      )}
      {...props}
