...
export class ApiClient {
  ...
  setGroupAssignments = (id: number, user_ids: number[]) => this.request<components['schemas']['GroupAssignmentsResponse']>(`/groups/${id}/assignments`, { method: 'PUT', body: JSON.stringify({ user_ids }) })
  // —— 设置 ——
  ...
  register = (b: components['schemas']['UserAuthRegister']) => this.request<components['schemas']['UserAuthResponse']>('/user/auth/register', { method: 'POST', body: JSON.stringify(b) })
  login = (b: components['schemas']['UserAuthLogin']) => this.request<components['schemas']['UserAuthResponse']>('/user/auth/login', { method: 'POST', body: JSON.stringify(b) })
  me = () => this.request<components['schemas']['User']>('/user/auth/me')
  listUserGroups = () => this.request<components['schemas']['Group'][]>('/user/groups')
  listUserKeys = (p?: ListParams) => this.request<components['schemas']['KeyListResponse']>('/user/keys', { params: toQuery(p) })
  createUserKey = (b: components['schemas']['KeyCreate']) => this.request<components['schemas']['KeyWithSecret']>('/user/keys', { method: 'POST', body: JSON.stringify(b) })
  updateUserKey = (id: number, b: components['schemas']['KeyUpdate']) => this.request<components['schemas']['Key']>(`/user/keys/${id}`, { method: 'PUT', body: JSON.stringify(b) })
  deleteUserKey = (id: number) => this.request<components['schemas']['DeletedResponse']>(`/user/keys/${id}`, { method: 'DELETE' })
  rotateUserKey = (id: number) => this.request<components['schemas']['KeyWithSecret']>(`/user/keys/${id}/rotate`, { method: 'POST' })
  getUserLogs = (params?: UserLogParams) => this.request<components['schemas']['LogsResponse']>('/user/logs', { params: toQuery(params) })
  getUserStats = (params?: UserStatParams) => this.request<components['schemas']['StatBucket'][]>('/user/stats', { params: toQuery(params) })
  redeem = (code: string) => this.request<components['schemas']['RedeemResponse']>('/user/redemptions', { method: 'POST', body: JSON.stringify({ code }) })
  listUserRedemptions = (p?: { page?: number; page_size?: number; sort?: string; order?: 'asc' | 'desc' }) => this.request<components['schemas']['RedemptionRecordListResponse']>('/user/redemptions', { params: toQuery(p) })
}
