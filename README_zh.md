...
- `compose.yml` — 生产编排：`db`（postgres:18-alpine，数据挂载在 `deploy/data/pg`）+ `redis`（redis:8-alpine，易失状态（协调 + 短时效验证码）——不持久化）+ `app`（单容器，非 root、配置只读挂载自 `deploy/config.toml`、健康检查）。当前默认镜像固定为 `v0.0.1-beta.14`；只有机器需要单独覆盖时才设置 `IMAGE` 或 `CONFIG_FILE`。
